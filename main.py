...
def start(message):
    ...
    print(Tname)
    markup.add(hhelp)
    bot.send_message(message.chat.id, "Данный бот создан для помощи в оплате товаров и услуг.",reply_markup=markup, parse_mode="html") #старт и пояснение
...
def Buy(massage):
    ...
    bot.send_message(massage.chat.id, "для оплаты перейдите по ссылке (!при покупке укажите свой telegram username!):", reply_markup=markup ,parse_mode="html")           #ссылка на покупку
...
def new_donation(event): 
       get(f'https://api.telegram.org/bot{bottoken}/sendMessage?chat_id={tg_user_id}&text=Новый донат:\n{event.username} - {event.amount} {event.currency}\n{event.message}')
       global name
